...
export async function evaluateTradeIdea({ mode, asset, strategy, signal, currentPrice, candles, macroCandles = [], indicators = {}, derivativesData = {}, orderBook = {}, pnlPercent = 0, marketType = 'FUTURES', openTrade = null }) {
    ...
    if (mode === 'ENTRY') {
        ...
        1. THE TRAILING PROFIT MANDATE: If the trade has reached the 75% profit tripwire, you MUST aggressively protect those gains. You are required to choose 'ADJUST_LIMITS' and trail the 'sl_price' deeply into profit territory. You must place the new Stop Loss at a level that locks in AT LEAST 50% to 75% of the current unrealized profit, guaranteeing a substantial win even if the market reverses.
