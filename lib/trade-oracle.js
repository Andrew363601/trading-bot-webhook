...
export async function evaluateTradeIdea({ mode, asset, strategy, signal, currentPrice, candles, pnlPercent = 0 }) {
    try {
      ...
      // We only send the last 150 candles to keep the request lightning fast
      ...
      if (mode === 'ENTRY') {
          ...
          If conviction is exceptionally high (> 85), you may recommend a leverage multiplier (e.g., 1.5 for 50% more size). Otherwise, return 1.0.
          `;
          expectedOutput = `{\n  "action": "APPROVE" | "VETO",\n  "conviction_score": 0-100,\n  "limit_price": [Optimal Entry Number],\n  "leverage_multiplier": 1.0 - 1.5,\n  "reasoning": "string"\n}`;
      } 
      ...
      // Fail-safe: If the AI crashes, we VETO new entries and HOLD emergencies to protect capital
