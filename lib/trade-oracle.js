...
export async function evaluateTradeIdea({ mode, asset, strategy, signal, currentPrice, candles, macroCandles = [], indicators = {}, derivativesData = {}, orderBook = {}, pnlPercent = 0, marketType = 'FUTURES', openTrade = null }) {
    ...
    if (mode === 'ENTRY') {
        ...
        prompt = `
        MANUAL SNIPER / TRIPWIRE REVIEW INITIATED: The user (or the automated 50% profit tripwire) has requested a tactical re-evaluation of this open ${openTrade.side} position on ${asset}.
        ...
        1. Capital preservation is priority one. If structural failure is imminent, execute a MARKET_CLOSE immediately.
        2. THE RISK-FREE RULE: If the trade is in profit and momentum is fading, choose ADJUST_LIMITS to lower the Take Profit and secure the bag early. However, if you do this, you MUST move the Stop Loss past the Entry Price ($${openTrade.entry_price}) into profit territory to guarantee a risk-free trade.
        3. If momentum is accelerating perfectly, choose HOLD to let profits run to the original target.
        `;
