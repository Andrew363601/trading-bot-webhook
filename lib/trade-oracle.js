...
export async function evaluateTradeIdea({ mode, asset, strategy, signal, currentPrice, candles, macroCandles = [], indicators = {}, derivativesData = {}, orderBook = {}, pnlPercent = 0, marketType = 'FUTURES', openTrade = null, recentHistory = [] }) {
    ...
    const sizingContext = marketType === 'SPOT' 
        ? "Since this is a SPOT market, leverage is not applicable. If conviction is exceptionally high (> 85), you may recommend a 'size_multiplier' (e.g., 1.5). For GOD-TIER setups (Conviction >= 90) with undeniable macro alignment, you are authorized to output a 5.0 to 10.0 'size_multiplier'. If you use a 5x-10x multiplier, your 'reasoning' MUST explicitly state why the macro structure justifies this massive size. Otherwise, return 1.0."
        : "Since this is a FUTURES market, if conviction is exceptionally high (> 85), you may recommend a 'size_multiplier' to increase position size. For GOD-TIER setups (Conviction >= 90) with undeniable macro breakout alignment and volume, you are authorized to output a 5.0 to 10.0 'size_multiplier' for maximum ROI. If you deploy a 5x-10x multiplier, your 'reasoning' MUST explicitly state why the macro structure justifies this massive sizing. Otherwise, return 1.0.";
    ...
    // 🧠 THE MEMORY MODULE (WITH ANTI-PTSD OVERRIDE)
    const memoryContext = `
