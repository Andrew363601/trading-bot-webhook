...
export async function evaluateTradeIdea({ mode, asset, strategy, signal, currentPrice, candles, macroCandles = [], indicators = {}, derivativesData = {}, orderBook = {}, pnlPercent = 0, marketType = 'FUTURES', openTrade = null, recentHistory = [], dynamicSizing = false, activeThesis = "" }) {
    ...
    // 🟢 THE FIX: Topographical Exit Mandates
    const regimeContext = `
    REGIME DECLARATION & SPLIT-BRAIN EXECUTION (CRITICAL):
    You are equipped with a Macro Topographical Map:
    - Macro POC (Point of Control): $${indicators.macro_poc || 'Unknown'} (The highest volume node over the last 150 macro periods).
    - Upper Macro Node (The Ceiling): $${indicators.upper_macro_node || 'None'}a
    - Lower Macro Node (The Floor): $${indicators.lower_macro_node || 'None'}
    - Macro CVD: ${indicators.macro_cvd || 'Unknown'} (Institutional accumulation/distribution over the last 50 macro periods).
    
    You MUST declare the current 'market_regime' in your JSON output:
    1. "CHOP" (Ranging): If price is tightly winding around the Macro POC and Macro CVD is relatively flat or mixed. 
       * CHOP RULES: You are STRICTLY FORBIDDEN from using the 'Elite Override Rule' to buy momentum or breakouts. Momentum in chop is a trap. You MUST VETO breakout signals. 
       * TOPOGRAPHICAL TARGETS: If Shorting the Upper Node, your Take Profit MUST be the Macro POC. If Buying the Lower Node, your Take Profit MUST be the Macro POC. Do not use arbitrary percentages.
    2. "TREND" (Trending): If price has violently broken away from the Macro POC, and Macro CVD is heavily directional.
       * TREND RULES: Hunt micro-pullbacks to VWAP to ride the momentum. Ignore mean-reversion signals. Aggressively trail stop-losses. The 'Elite Override Rule' is fully active. Target the next historical Node on the map.
    `;
    ...
    - Technical Indicators: ${JSON.stringify(indicators)}
      * CRITICAL CVD CHECK: If Micro CVD (Cumulative Volume Delta) is dropping heavily while price is rising, this is a BEARISH DIVERGENCE (Institutions are quietly selling into retail liquidity). You MUST VETO LONGS and look for a Short.
      * Use VWAP and ATR to pinpoint exact limit order entries on micro-pullbacks, but do not be overly stingy and miss the trade.
    ...
    3. The Shadow Portfolio (VETO Context): You will see 'SHADOW VETO' entries in your memory. If you recently vetoed multiple Longs anticipating a crash, and a SHORT signal now fires, use this shadow context to drastically increase your Conviction. Trust your prior analysis.

    CRITICAL MANDATE (ANTI-PTSD PROTOCOL): Use this memory to avoid whipsawing, BUT DO NOT let recency bias paralyze you in a TREND regime. 
    *THE ELITE OVERRIDE RULE:* If you invoke this override to take a trade immediately after a recent loss, the new setup MUST command an elite Conviction Score of 85 or higher, AND the market_regime MUST be "TREND". If the regime is "CHOP", you are forbidden from using this override for breakout momentum.
    `;

    // 🟢 THE FIX: The Short TTL Leash
    const thesisContext = `
    ...
    - THE VIRTUAL TRAP (GHOST ORDER): If you are VETOING a premature signal but know exactly where you want to enter (e.g., catching a wick at the edge of a CHOP range), you can set a Virtual Trap. Output a 'trap_side', 'trap_price', and a STRICTLY SHORT 'trap_expires_in_minutes' (MAXIMUM 10 to 15 minutes). Do not let traps sit for hours; market regimes shift fast.
    ...
    if (mode === 'ENTRY') {
        ...
        1. If momentum is standard or chopping, set "order_type" to "LIMIT". Calculate the optimal 'limit_price', but do not demand absolute perfection to the penny. If Risk/Reward is favorable, get in the trade.
        2. If momentum is EXPLOSIVE (and the regime is TREND), do not risk missing the trade. Set "order_type" to "MARKET" to execute immediately. Set 'limit_price' to the current market price ($${currentPrice}) as a reference point.
        
        CRITICAL EXPECTANCY METRICS:
        You must calculate 'fill_expectancy' (Keep this short, 5-15 mins maximum based on current ATR and volume). If using a MARKET order, set to 0.
        ...
        // 🟢 THE FIX: Aggressive Trend Reversal Entry
        THE REVERSAL ENTRY PROTOCOL (MARKET vs LIMIT):
        If you APPROVE the reversal, decide how to enter:
        1. If the 'market_regime' is "TREND", you MUST set "order_type" to "MARKET" to flip the position instantly. Do not risk missing the momentum with a limit order. Set 'limit_price' to the current market price ($${currentPrice}).
        2. If the 'market_regime' is "CHOP", set "order_type" to "LIMIT" and calculate the optimal limit order entry price.
        
        CRITICAL EXPECTANCY METRICS:
        You must calculate 'fill_expectancy' (Keep this short, maximum 15 minutes). If using a MARKET order, set to 0.
        You must calculate 'tp_expectancy' (how many minutes until the primary impulse hits the Take Profit).
    ...
    if (activeModel.provider === 'openrouter') {
        ...
        llmBody = {
            ...
            messages: [
                { role: 'system', content: "You are an elite quantitative execution risk manager. Output ONLY raw, valid JSON." },
    ...
    while (attempts < maxRetries) {
        try {
            const resp = await fetch(llmUrl, { method: 'POST', headers: llmHeaders, body: JSON.stringify(llmBody) });
            
            if (!resp.ok) {
                const errorText = await resp.text();
                if (resp.status === 503 || resp.status === 429) {
                    attempts++;
                    console.warn(`[ORACLE WARN] LLM API Congestion (Status ${resp.status}). Retrying... Attempt ${attempts}/${maxRetries}`);
                    if (attempts >= maxRetries) throw new Error(`Max retries reached. Last error: ${errorText}`);
                    await new Promise(resolve => setTimeout(resolve, 1500 * attempts));
                    continue; 
                }
                throw new Error(`LLM API rejected (${activeModel.provider}): ${resp.status} - ${errorText}`);
            }
            ...
            if (!cleanJsonString) {
                throw new Error(`Empty LLM response returned from ${activeModel.provider}`);
            }
