...
export async function evaluateTradeIdea({ mode, asset, strategy, signal, currentPrice, candles, macroCandles = [], indicators = {}, derivativesData = {}, orderBook = {}, pnlPercent = 0, marketType = 'FUTURES', openTrade = null }) {
    ...
    if (mode === 'ENTRY') {
        ...
        // THE NEW SNIPER MODE
        prompt = `
        MANUAL SNIPER REVIEW INITIATED: The user has explicitly requested a tactical re-evaluation of this open ${openTrade.side} position on ${asset}.
        ...
        CRITICAL DIRECTIVE: Capital preservation is your absolute highest priority.
        1. If the original thesis is completely dead or structural failure is imminent, execute a MARKET_CLOSE immediately. Do not hold a losing position on hope.
        2. If the thesis is valid but our current TP/SL targets are poorly placed based on the *new* Order Book walls or shifting Macro S/R, choose ADJUST_LIMITS and provide precise new 'tp_price' and 'sl_price' coordinates.
        3. If everything is perfectly fine and the current targets are optimal, choose HOLD.
        `;
