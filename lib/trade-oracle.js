...
export async function evaluateTradeIdea({ mode, asset, strategy, signal, currentPrice, candles, pnlPercent = 0, marketType = 'FUTURES', openTrade = null }) {
    ...
    // --- THE FIX: We added tp_price and sl_price to the expected output ---
    if (mode === 'ENTRY') {
        prompt = `
        ...
        CRITICAL: You must also calculate a dynamic 'tp_price' (Take Profit) just before major overhead resistance, and a dynamic 'sl_price' (Stop Loss) just below major structural support to ensure a high-probability risk/reward ratio.
        
        ${sizingContext}
        `;
        expectedOutput = `{\n  "action": "APPROVE" | "VETO",\n  "conviction_score": 0-100,\n  "limit_price": [Optimal Entry Number],\n  "tp_price": [Optimal Take Profit Number],\n  "sl_price": [Optimal Stop Loss Number],\n  "size_multiplier": 1.0 - 1.5,\n  "reasoning": "string"\n}`;
        ...
        CRITICAL WARNING - AVOID WHIPSAWING: Algorithmic strategies often spam continuous reversal signals during temporary market chop. 
        Analyze the recent 150 candles. You must VETO this new signal UNLESS there is a massive, undeniable structural trend reversal that guarantees our current ${openTrade.side} position is doomed.
        If it is just temporary chop, micro-pullbacks, or ranging price action, VETO the signal to hold our original ${openTrade.side} position and let the mathematical Stop Loss handle risk.
        
        If you APPROVE the reversal, calculate the optimal limit order entry price to execute the flip. Calculate a dynamic 'tp_price' and 'sl_price' based on immediate market structure.
