...
const TIER_MAP = {
  ...
  coinglass_oi_momentum_v4: {
    ...
    reason: 'Price×OI quadrant on REAL aggregated OI (CoinGlass, current macro-TF window): long fuel = price↑+OI↑ or liquidation-flush bounce; short fuel = price↓+OI↑ or squeeze exhaustion. Synthetic-venue OI is structural noise — CoinGlass is the source.',
    description: 'Open Interest momentum — is capital flowing into this move?'
  },
  coinglass_funding_rate_reversion_v4: {
    tier: 3, tierLabel: 'Tier 3: Energy (Derivatives)', tierColor: '#f59e0b',
    stage: 1, stageLabel: 'Stage 1: Quick Scan', timeframe: 'MACRO_TIMEFRAME',
    reason: 'OI-weighted funding (CoinGlass, current macro-TF window): extreme positive = crowded longs (short edge), extreme negative = crowded shorts (long edge). Direction-aware.',
  ...
  get_atr_levels: {
    tier: 2, tierLabel: 'Tier 2: Terrain (S/R)', tierColor: '#3b82f6',
    stage: 'SYSTEM', stageLabel: 'Core System', timeframe: 'MACRO_TIMEFRAME (basis) / TRIGGER (cadence)',
    reason: 'Macro-ATR stops/targets — 1.5x–2.0x macro-ATR buffer (3x only if structure demands), HVN-based TP',
