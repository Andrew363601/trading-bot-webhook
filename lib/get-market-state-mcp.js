...
// 🟢 THE FIX: Keyless Cross-Asset Macro Fetcher
...
async function fetchMicrostructure(asset, triggerCandles, macroCandles, apiKey, secret) {
    try {
        ...
        let cvd_sequence = []; // 🟢 THE FIX: The Predictive Tape Array
        ...
        return { 
            indicators: { 
                ...
                cvd_sequence: cvd_sequence, // 🟢 Exposed Sequential Momentum Tape
...
export async function getMarketStateMCP(args) {
    ...
    try {
        // 🟢 THE FIX: Concurrent Cross-Asset Macro Fetching
        const [candles1H, candles15M, candles5M, sp500, dxy] = await Promise.all([
            ...
            fetchMacroAsset('%5EGSPC'), // S&P 500
            fetchMacroAsset('DX-Y.NYB') // US Dollar Index
        ]);
        ...
        return {
            ...
            multi_timeframe_cvd: {
                ...
                "5M_Sequence": micro5M.indicators.cvd_sequence // 🟢 The Tape
            },
            volatility_atr: { 
                "1H": micro1H.indicators.current_atr,
                "15M": micro15M.indicators.current_atr,
