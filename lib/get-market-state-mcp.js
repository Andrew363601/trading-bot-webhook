...
export async function getMarketStateMCP(args) {
    ...
    // 🟢 THE FIX: Grabbing the dynamic timeframes from the Agent
    const macroTf = args.macro_tf || 'ONE_HOUR';
    const triggerTf = args.trigger_tf || 'FIVE_MINUTE';

    try {
        const [macroCandles, triggerCandles] = await Promise.all([
            fetchCoinbaseData(symbol, macroTf, apiKeyName, apiSecret),
            fetchCoinbaseData(symbol, triggerTf, apiKeyName, apiSecret)
        ]);

        if (!macroCandles || !triggerCandles) throw new Error("Failed to fetch candle data from exchange.");

        const microstructure = await fetchMicrostructure(symbol, triggerCandles, macroCandles, apiKeyName, apiSecret);
        const currentPrice = triggerCandles[triggerCandles.length - 1].close;
        ...
        return {
            ...
            price_action: {
                recent_closes: triggerCandles.slice(-5).map(c => c.close),
                recent_volumes: triggerCandles.slice(-5).map(c => c.volume)
            },
            volume_profile: microstructure.indicators,
            order_book_depth: microstructure.orderBook,
            derivatives_premium: microstructure.derivativesData
        };
