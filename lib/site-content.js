// lib/site-content.js
// Fetches landing-page content from Supabase with hardcoded fallback.
// The fallback ensures the page is never blank even if Supabase is unreachable.

export const FALLBACK_CONTENT = {
  hero: {
    title: "Stop Trading.",
    titleGradient: "Start Executing.",
    subtitle: "The world's first autonomous, self-learning quantitative trading agent built for the retail trader. Don't just automate your strategy. Arm it with institutional-grade AI.",
    ctaConnect: "Deploy Your Agent",
    ctaDashboard: "Launch Dashboard",
    // PUSH AM13: cardless 100K challenge trial
    trialText: "Free 100K Challenge · No card · First 30 days free"
  },
  features: [
    { title: "Institutional Reasoning", body: "Nexus AI scans the 6H Macro Tide, the 1H Trend, and the 5M Tape. If a setup tries to catch a falling knife, Nexus AI vetoes the trade to protect your capital." },
    { title: "Agentic Reflection", body: "Nexus AI runs a post-mortem on every closed trade. If a setup fails, it extracts the math and writes a permanent rule to its Core Memory. It learns from its trauma." },
    { title: "Multi-TF X-Ray", body: "Real-time Level 2 spoof detection, volume node mapping, and structural fractal stop-losses. It calculates the exact Reward-to-Risk ratio before entering." }
  ],
  differentiators: [
  ...
  testimonials: [
    { name: 'Marcus T.', plan: 'Retail', quote: 'I was tired of rewriting Pine Script every time I changed my mind. Now Nexus handles execution while I focus on higher-level strategy.', total_pnl: 4850, closed_trades: 37 },
    { name: 'Sarah K.', plan: 'Pro', quote: 'I\'ve been running my ETH scalper for 3 weeks and it\'s already up $2,100. The 5-tier confluence catches things my old setup never saw.', total_pnl: 2100, closed_trades: 18 },
    { name: 'David L.', plan: 'Retail', quote: 'The Accountant Protocol saved me from a 3x loss on my first week alone. Worth every cent just for that safety net.', total_pnl: 1250, closed_trades: 12 }
  ],
};

export async function fetchSiteContent(supabase) {
  try {
    ...
    // Merge fallback to fill any missing sections
    return { ...FALLBACK_CONTENT, ...contentMap };
  } catch (e) {
    console.warn('[SITE] Using fallback:', e.message);
    return FALLBACK_CONTENT;
  }
}