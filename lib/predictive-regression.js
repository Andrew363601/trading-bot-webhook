...
async function fetchModelRow(tenantId, asset, regime, strategy, tfPair) {
    let q = supabase
        ...
        .select('asset, regime, strategy, sample_count, expected_pnl_mean, expected_pnl_std, feature_importance, metrics, last_trained, model_params, suggested_params')
        .eq('asset', asset)
        .eq('regime', regime)
    ...
    try {
        ...
        if (!row) {
            ...
            result = {
                ...
                // 🟢 AM7 — Layer 3: the trainer's top params profile for this
                // bucket, delivered to the wake so the agent weighs it like a
                // signal prior when setting tp/sl/tripwire/trail.
                suggestedParams: row.suggested_params || null,
                scope: row._scope,
                tier: row._tier,
