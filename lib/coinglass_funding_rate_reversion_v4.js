...
export async function coinglass_funding_rate_reversion_v4(symbol, k_minutes = 1440, interval) {
    ...
    // Interval is REQUIRED on this endpoint — omitting it is a hard 400 error.
    try {
        const intervalParam = cgInterval(interval, '1h');
        const data = await cgGet(`api/futures/funding-rate/history?symbol=${cgInstrument(symbol)}&exchange=Binance&interval=${intervalParam}&limit=168`);

        const rows = Array.isArray(data) ? data.slice(-k_minutes) : [];
