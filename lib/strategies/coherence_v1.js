...
export async function run(macroCandles, triggerCandles, parameters) {
    ...
    const marketType = parameters?.market_type || 'FUTURES';
    ...
    // --- THE TELEMETRY FIX ---
    // Define the telemetry payload using the rich data the helper function returns
    const currentTelemetry = {
        macro_mci: macroMCI.mci,
        trigger_mci: triggerMCI.mci,
        di_plus: triggerMCI.di_plus ? parseFloat(triggerMCI.di_plus.toFixed(2)) : 0,
        di_minus: triggerMCI.di_minus ? parseFloat(triggerMCI.di_minus.toFixed(2)) : 0
    };

    // Resonance Check
    const isResonant = macroMCI.mci > 0.60 && triggerMCI.is_resonant;
    
    // If conditions aren't met, return a null signal safely WITH the new telemetry object
    if (!isResonant) {
        return { 
            signal: null, 
    ...
    // Return the Standardized Decision Envelope (Trade Fired!)
    return {
        signal: side,
