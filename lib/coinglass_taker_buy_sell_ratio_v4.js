export async function coinglass_taker_buy_sell_ratio_v4(symbol, interval = '15m') {
    // Market Context: High-Velocity Breakdown, Momentum Breakout [5, 15]
    try {
        const tf = interval || '15m';
        const res = await fetch(`https://open-api-v4.coinglass.com/api/futures/v2/taker-buy-sell-volume/history?symbol=${symbol}&exchange=Binance&interval=${tf}&limit=168`, {
        ...
        const allZero = rows.length > 0 && rows.every(d => !Number(d.buyVol) && !Number(d.sellVol));
        if (allZero) {
