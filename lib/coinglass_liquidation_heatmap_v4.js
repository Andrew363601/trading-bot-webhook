...
import { cgInstrument, cgInterval } from './coinglass-api.js';
...
export async function coinglass_liquidation_heatmap_v4(symbol, interval) {
  try {
    // Coinglass aggregated liquidation heatmap model (time x price grid).
    // STARTUP plan floor: never request sub-30m intervals.
    const iv = cgInterval(interval);
    // range "3d" keeps payloads reasonable; the front-end only renders the
    // visible window anyway.
    const url = `https://open-api-v4.coinglass.com/api/futures/liquidation/aggregated-heatmap/model?symbol=${cgInstrument(symbol)}&interval=${iv}&range=3d`;
  ...
  try {
    ...
    if (snap.status === 'success') {
      return { status: 'success', mode: 'bands', interval: cgInterval(interval), heatmap: snap.heatmap, current_price: snap.current_price };
    }
    return snap;
  } catch (e) {
