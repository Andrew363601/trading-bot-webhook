export async function coinglass_global_long_short_sentiment_v4(symbol, k_hours = 168, interval = '1h') {
    ...
    try {
        const res = await fetch(`https://open-api-v4.coinglass.com/api/futures/global-long-short-account-ratio/history?symbol=${symbol}&interval=${interval}&limit=168`, {
        ...
        const series = period_data.map(d => {
            ...
            return { time: tNum, value: Number(d.shortAccounts) ? Number(d.longAccounts) / Number(d.shortAccounts) : 0 };
