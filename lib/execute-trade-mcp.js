...
import { buildRadarChartUrl } from './discord-chart.js'; // Note path change
...
// 🟢 THE FIX: Converted from an HTTP Handler to a raw Async Function for MCP
   ...
   if (!coinbaseProduct.includes('-')) {
    ...
    if (openTrade) {
      if (isClosing) {
        ...
        return { status: "ignored_already_open", product: coinbaseProduct }; // 🟢 Returns plain object
      }
    } else {
    ...
    // 🟢 THE FIX: Write the AI's active thesis back to the UI Database
    ...
    return { status: executionStatus, product: coinbaseProduct, price: executionPrice }; // 🟢 Returns plain object
    ...
    return { error: err.message }; // 🟢 Returns plain object
  }
}