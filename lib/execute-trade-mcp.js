   ...
   if (!coinbaseProduct.includes('-')) {
    ...
    if (openTrade) {
      if (isClosing) {
        ...
        let safeExitPrice = parseFloat(executionPrice) || 0;

        // 🟢 THE FIX: Fallback for $0 exit price (e.g., Hermes CLOSE without 'price' field)
        // For PAPER trades that close via executeTradeMCP, use SL/TP from the open trade or candle close
        if (!safeExitPrice || safeExitPrice === 0) {
          const fallbackFromTrade = parseFloat(openTrade.sl_price || openTrade.tp_price || 0);
          const fallbackFromCandle = recentCandles.length > 0
            ? recentCandles[recentCandles.length - 1].close
            : 0;
          safeExitPrice = fallbackFromTrade || fallbackFromCandle || 0;
          console.log(`[PNL SAFETY] CLOSE without exit price. Trade: ${openTrade.id}, Side: ${openTrade.side}, Entry: $${safeEntryPrice}, Using fallback: $${safeExitPrice} (SL: ${openTrade.sl_price}, TP: ${openTrade.tp_price}, candle close: ${fallbackFromCandle})`);
        }
        ...
        // 🟢 PnL SANITY CHECK: Verify PnL sign matches price movement direction
        // Prevents inverted win/loss classification from bad exit prices
        let finalPnl = parseFloat(pnl.toFixed(4));
        if (safeExitPrice > 0 && safeEntryPrice > 0 && safeExitPrice !== safeEntryPrice) {
          const priceMovedUp = safeExitPrice > safeEntryPrice;
          const expectedPositivePnl = (openTrade.side === 'BUY' && priceMovedUp) || (openTrade.side === 'SELL' && !priceMovedUp);
          const expectedNegativePnl = (openTrade.side === 'BUY' && !priceMovedUp) || (openTrade.side === 'SELL' && priceMovedUp);
          if ((finalPnl > 0 && expectedNegativePnl) || (finalPnl < 0 && expectedPositivePnl)) {
            console.error(`[PNL SANITY] INVERTED PnL! Trade: ${openTrade.id}, Side: ${openTrade.side}, Entry: $${safeEntryPrice}, Exit: $${safeExitPrice}, Raw: $${finalPnl} vs expected ${expectedPositivePnl ? 'POS' : 'NEG'}. Forcing to 0.`);
            finalPnl = 0;
          }
        }
        ...
        try {
            const { error: updateError } = await supabase.from('trade_logs').update({ exit_price: safeExitPrice, pnl: finalPnl, exit_time: new Date().toISOString(), reason: updatedReason }).eq('id', openTrade.id);
            
            if (updateError) {
                console.error("[SUPABASE ERROR] TRADE LOG UPDATE FAILED:", updateError.message);
            } else {
                try {
                    await supabase.from('scan_results').insert([{ strategy: strategyId, asset: rawSymbol, status: 'CLOSED', telemetry: { macro_regime_oracle: `AGENT CLOSED POSITION`, oracle_reasoning: updatedReason, open_pnl: finalPnl, open_position: "NONE" } }]);
        ...
        await sendDiscordAlert(tenantId, {
            ...
            description: `**Exit Price:** $${safeExitPrice}\n**Realized PnL:** $${finalPnl}${entryText}${tpText}${slText}\n**Trigger:** ${tradeReason || 'Signal Reversal'}`, 
            color: finalPnl >= 0 ? 5763719 : 15548997,
