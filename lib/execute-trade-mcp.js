...
async function ensureDerivativesFunds(generateToken, requiredMargin) {
    try {
        ...
        // 3. Fund CFM futures directly from spot balance
        const fundPath = '/api/v3/brokerage/cfm/fund';
        const fundResp = await fetch(`https://api.coinbase.com${fundPath}`, {
            method: 'POST',
            headers: { 'Authorization': `Bearer ${generateToken('POST', fundPath)}`, 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        if (!fundResp.ok) {
            const fundErr = await fundResp.text();
            console.error(`[AUTO-FUND] CFM fund rejected: ${fundErr}`);
            return { funded: false, reason: `CFM fund failed: ${fundErr}` };
        }

        console.log(`[AUTO-FUND] Funded $${transferAmount.toFixed(2)} from spot into CFM futures. Waiting 1.5s for propagation...`);
