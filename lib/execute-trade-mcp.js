...
async function ensureDerivativesFunds(generateToken, requiredMargin) {
    try {
        ...
        // 3. Get portfolio UUID for deposit into derivatives
        ...
        if (portResp.ok) {
            ...
            if (portfolios.length > 0) {
                // Use the first portfolio (typically DEFAULT which contains CFM)
                portfolioUuid = portfolios[0].uuid || portfolios[0].portfolio_uuid || portfolios[0].id;
                console.log(`[AUTO-FUND] Using portfolio: ${portfolios[0].name || portfolios[0].type || portfolios[0].id} (${portfolioUuid})`);
            }
        }

        if (!portfolioUuid) {
            return { funded: false, reason: 'No portfolio UUID available for transfer' };
        }

        // 4. Transfer from spot into derivatives portfolio
        ...
        if (!transferResp.ok) {
            ...
            console.error(`[AUTO-FUND] Portfolio transfer rejected: ${transferErr}`);
            return { funded: false, reason: `Transfer failed: ${transferErr}` };
        }

        console.log(`[AUTO-FUND] Transferred $${transferAmount.toFixed(2)} into portfolio. Waiting 1.5s for propagation...`);
