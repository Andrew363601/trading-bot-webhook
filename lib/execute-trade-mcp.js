...
async function ensureDerivativesFunds(generateToken, requiredMargin) {
    try {
        ...
        // 3. Get CFM portfolio UUID with flexible search
        ...
        const portfolios = portData.portfolios || [];

        // Debug: Log portfolio structure to understand Coinbase's naming
        if (portfolios.length > 0) {
            console.log(`[AUTO-FUND] Available portfolios: ${JSON.stringify(portfolios.map(p => ({ type: p.type, name: p.name, uuid: p.uuid || p.portfolio_uuid || p.id })))}`);
        }

        // Cascading portfolio search: try multiple patterns
        let cfmPortfolio = portfolios.find(p =>
            p.type === 'FUTURES' ||
            (p.type && p.type.toUpperCase().includes('FUTURES')) ||
            (p.type && p.type.toUpperCase().includes('CFM')) ||
            (p.type && p.type.toUpperCase().includes('INTX')) ||
            (p.type && p.type.toUpperCase().includes('DERIVATIVE')) ||
            (p.name && p.name.toUpperCase().includes('FUTURES')) ||
            (p.name && p.name.toUpperCase().includes('CFM')) ||
            (p.name && p.name.toUpperCase().includes('INTX')) ||
            (p.name && p.name.toUpperCase().includes('DERIVATIVE'))
        );

        // Fallback: if no derivatives portfolio found, pick the non-default portfolio
        if (!cfmPortfolio) {
            const defaultPort = portfolios.find(p => p.default === true || p.type === 'DEFAULT');
            cfmPortfolio = portfolios.find(p => p !== defaultPort);
            if (cfmPortfolio) {
                console.log(`[AUTO-FUND] Using fallback portfolio: ${cfmPortfolio.name || cfmPortfolio.type || cfmPortfolio.id} (${cfmPortfolio.uuid || cfmPortfolio.portfolio_uuid || cfmPortfolio.id})`);
            }
        }

        if (!cfmPortfolio) {
            return { funded: false, reason: `No CFM futures portfolio found among ${portfolios.length} portfolios` };
        }
    ...
    if (!isPaper) {
      ...
      if (!isClosing && marketType !== 'SPOT') {
          ...
          if (estimatedMargin > 0) {
              fundResult = await ensureDerivativesFunds(generateToken, estimatedMargin);
          }
      }
      if (fundResult && !fundResult.funded) {
          console.warn(`[AUTO-FUND] Pre-funding check: ${fundResult.reason}. Order will proceed and may fail if insufficient.`);
      } else if (fundResult && fundResult.amount) {
      ...
      // Handle order rejection — with retry for INSUFFICIENT_FUNDS after a transfer
      if (!resp.ok || result.success === false || result.error_response) {
        ...
        // Retry once if we just transferred funds and got insufficient funds (propagation delay)
        if (isInsufficientFunds && fundResult && fundResult.amount) {
            console.log(`[AUTO-FUND] Order rejected with INSUFFICIENT_FUNDS after transfer. Waiting 3s and retrying...`);
            await new Promise(resolve => setTimeout(resolve, 3000));
            
            const retryResp = await fetch(`https://api.coinbase.com${path}`, { method: 'POST', headers: { 'Authorization': `Bearer ${generateToken('POST', path)}`, 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
            const retryResult = await retryResp.json();
            
            if (retryResp.ok && retryResult.success !== false && !retryResult.error_response) {
                result = retryResult;
                resp = retryResp;
                console.log(`[AUTO-FUND] Retry succeeded after fund propagation.`);
            } else {
                const retryErr = retryResult.error_response?.preview_failure_reason || retryResult.error_response?.error || retryResult.failure_reason?.error_message || JSON.stringify(retryResult);
                console.error(`[AUTO-FUND] Retry also failed: ${retryErr}`);
                if (pendingTradeId) {
                    try {
                ...
                throw new Error(`Coinbase Order Rejected (after fund retry): ${retryErr}`);
            }
        } else {
            if (pendingTradeId) {
                try {
