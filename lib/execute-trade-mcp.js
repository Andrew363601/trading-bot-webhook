...
export async function executeTradeMCP(data) {
  try {
   ...
   if (!coinbaseProduct.includes('-')) {
    ...
    // 🟢 THE FIX: Moved isForcedExit to the top so we can intercept manual UI closes
    ...
    // 🟢 THE FIX: If your UI forced a close, physically flip the side to ensure the database logs it properly
    ...
    // 🟢 THE FIX: Anti-Inversion Safety Net
    if (tpPrice && slPrice) {
    ...
    if (openTrade) {
      ...
      if (isForcedExit) return { status: "already_closed_natively", product: coinbaseProduct };
