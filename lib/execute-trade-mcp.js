  ...
  let coinbaseProduct = rawSymbol;
   if (!coinbaseProduct.includes('-')) {
    ...
    if (!isPaper) {
      ...
      // 🟢 THE FIX V3: For closing orders, only fall back to SL price when executionPrice is
      // truly invalid (null/NaN/zero). Do NOT overwrite a valid market price just because
      // it happens to equal incomingPrice — that is the correct behavior when the retry
      // loop couldn't get a better fill price from the exchange.
      if (isClosing && openTrade?.sl_price && (!executionPrice || isNaN(executionPrice) || executionPrice === 0)) {
          console.warn(`[EXECUTION SYNC] Closing order fill price unavailable (was: $${executionPrice}). Falling back to SL price: $${openTrade.sl_price}`);
