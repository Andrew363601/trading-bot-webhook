  ...
  try {
    ...
    if (tenantId) {
        try {
            ...
            if (!apiKeyName || !apiSecret) {
                console.error(`[EXECUTE] retrieveAPIKey returned empty keys for tenant ${tenantId}. Falling back to ENV.`);
                throw new Error("Empty keys returned");
            }
        } catch (e) {
            console.error(`[EXECUTE] Key retrieval failed for tenant ${tenantId}: ${e.message}. Falling back to ENV.`);
   ...
   if (!coinbaseProduct.includes('-')) {
    ...
    if (!isPaper) {
      ...
      // 🟢 THE FIX 1: Ripped out the obsolete UUID Auto-Funding code completely
