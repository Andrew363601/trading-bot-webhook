  ...
  try {
   ...
   if (!coinbaseProduct.includes('-')) {
    ...
    if (!isPaper) {
      ...
      // For closing orders, fallback to the open trade's SL price if available (more accurate than incomingPrice)
      if ((!executionPrice || isNaN(executionPrice) || executionPrice === 0) && isClosing && openTrade?.sl_price) {
          console.log(`[EXECUTION SYNC] Using open trade SL price as fill fallback: $${openTrade.sl_price}`);
