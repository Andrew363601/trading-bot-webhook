...
export async function executeTradeMCP(data) {
  try {
   ...
   if (!coinbaseProduct.includes('-')) {
    ...
    // 🟢 THE FIX: Deterministic DB Insert BEFORE hitting the exchange
    ...
    if (!isPaper) {
      ...
      // 🟢 THE FIX: Tagging the Exchange Order with the DB ID
      ...
      if (!resp.ok || result.success === false || result.error_response) {
          // Rollback the pending DB row if the exchange rejects it
      ...
      // Update the pending DB record with the exact execution fill price
      if (pendingTradeId) {
      ...
      if (!isClosing && orderType === 'MARKET' && tpPrice && slPrice) {
          ...
          try {
              // 🟢 THE FIX: Tagging the OCO Brackets with the exact DB ID
              const ocoPayload = {
