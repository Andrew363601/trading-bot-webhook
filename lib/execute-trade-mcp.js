  ...
  try {
   ...
   if (!coinbaseProduct.includes('-')) {
    ...
    if (!isPaper) {
      ...
      // 🟢 THE FIX: Force fetch exact execution price if it's not immediately returned
      if (result.success_response?.average_price) {
          ...
          await new Promise(resolve => setTimeout(resolve, 1000)); // Short breath for ledger sync
      ...
      if (isClosing) {
        ...
        if (updateError) {
            console.error("[TRADE LOG UPDATE FAILED]:", updateError.message);
             ...
             // 🟢 UI Audit Sync: AI commanded a close, so we write the receipt to the UI
