...
export async function executeTradeMCP(data) {
  try {
   ...
   if (!coinbaseProduct.includes('-')) {
    ...
    // 🟢 THE UPGRADE: Capture AI's Dynamic Multiplier and Trap Commands
    const qtyMultiplier = parseFloat(data.qty_multiplier || 1);
    const rawOrderType = data.order_type || data.execution || 'MARKET';
    const orderType = (rawOrderType === 'LIMIT_TRAP' || rawOrderType === 'LIMIT') ? 'LIMIT' : 'MARKET';
    ...
    // 🟢 AI'S INTENDED MATH: Let Hermes calculate what it WANTS to do
    let baseQty = parseFloat(data.qty || 1);
    let intendedQty = Math.max(1, Math.round(baseQty * qtyMultiplier));
    
    // 🟢 HARDCODED RISK OVERRIDE: Force all new entries to exactly 1 contract
    let orderQty = 1; 
    
    if (isClosing) {
        // CRITICAL: If closing/reversing, we MUST use the exact open quantity from the DB so we don't strand contracts
        orderQty = Math.max(1, Math.round(parseFloat(openTrade.qty || 1)));
    }
    ...
    if (tpPrice && slPrice) {
        if (side === 'BUY' && slPrice > tpPrice) {
            console.log(`[SAFETY NET] AI inverted BUY targets. Swapping SL ($${slPrice}) and TP ($${tpPrice}).`);
            const temp = slPrice; slPrice = tpPrice; tpPrice = temp;
        } else if (side === 'SELL' && slPrice < tpPrice) {
            console.log(`[SAFETY NET] AI inverted SELL targets. Swapping SL ($${slPrice}) and TP ($${tpPrice}).`);
            const temp = slPrice; slPrice = tpPrice; tpPrice = temp;
        }
    }
    ...
    if (!isClosing && !isForcedExit) {
        // 🟢 THE UPGRADE: Added order_type so Watchdog correctly identifies LIMIT Traps
        const { data: newTrade, error: insertErr } = await supabase.from('trade_logs').insert([{
            symbol: rawSymbol, side: side, entry_price: executionPrice, execution_mode: mode, strategy_id: strategyId, version: version, qty: orderQty, leverage: leverage, market_type: marketType, tp_price: tpPrice, sl_price: slPrice, reason: tradeReason, order_type: orderType
        }]).select();
    ...
    if (openTrade) {
      ...
      const actionTitle = orderType === 'LIMIT' ? `⏳ Limit Trap Placed: ${rawSymbol}` : `🚀 New Position Opened: ${rawSymbol}`;
      const targetText = (tpPrice && slPrice) ? `\n**Target TP:** $${tpPrice}\n**Target SL:** $${slPrice}` : `\n**Targets:** Dynamic`;
      const sizingText = qtyMultiplier !== 1 ? `\n**Multiplier:** ${qtyMultiplier}x (AI Intended: ${intendedQty}, Hardcoded Execution: 1)` : '';

      await sendDiscordAlert({
          title: actionTitle, 
          description: `**Side:** ${side}\n**Entry Price:** $${executionPrice}\n**Qty:** ${orderQty}${sizingText}\n**Mode:** ${mode}${targetText}${rationaleText}`, 
          color: 3447003,
          fields: standardFields,
          imageUrl: chartUrl
      }); 
