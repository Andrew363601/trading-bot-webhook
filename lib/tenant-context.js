...
/**
 * 🪟 PLAN GATE — concurrent active strategies.
 * Source of truth for "how many strategies is this tenant allowed to run at once?"
 *
 * Resolution order:
 *   1. tenants.max_concurrent_strategies   (per-tenant override, e.g. ADMIN bumps)
 *   2. Default by billing_tier             (FREE_TRIAL=1, RETAIL=3, PRO=10, INSTITUTIONAL=20)
 *
 * Returns { limit: number, active: number, hasRoom: boolean, tier: string }.
 * Tier defaults live in TIER_CONCURRENT_LIMITS so the SaaS plan ceilings are
 * trivial to retune in one place.
 */
export const TIER_CONCURRENT_LIMITS = {
    FREE_TRIAL: 1,
    RETAIL: 3,
    PRO: 10,
    INSTITUTIONAL: 20,
...
export async function getConcurrentStrategyQuota(tenantId) {
    const supabase = getTenantClient();
    try {
        const { data: tenant } = await supabase
            .from('tenants')
            .select('max_concurrent_strategies, billing_tier')
            .eq('id', tenantId)
            .single();

        const tier = (tenant?.billing_tier || 'FREE_TRIAL').toUpperCase();
        const tierDefault = TIER_CONCURRENT_LIMITS[tier] ?? TIER_CONCURRENT_LIMITS.FREE_TRIAL;
        // Per-tenant override beats the tier default.
        const limit = (typeof tenant?.max_concurrent_strategies === 'number' && tenant.max_concurrent_strategies > 0)
            ? tenant.max_concurrent_strategies
            : tierDefault;
        ...
        return { limit, active, hasRoom: active < limit, tier };
    } catch (e) {
        // Fail-safe: assume FREE_TRIAL limit so we never accidentally unblock more than 1.
        console.warn(`[QUOTA] Could not resolve concurrent quota for ${tenantId}: ${e.message}`);
        return { limit: TIER_CONCURRENT_LIMITS.FREE_TRIAL, active: 0, hasRoom: true, tier: 'FREE_TRIAL' };
    }
}
...
export async function isTenantBillingActive(tenantId) {
    ...
    try {
        ...
        // 2) tenants.subscription_active is the fast-path flag written by the
        //    webhook. billing_tier lets us recognise admin/internal accounts that
        //    don't have a real Stripe subscription row.
        const { data: tenant } = await supabase
            .from('tenants')
        ...
        const now = Date.now();
        const tier = (tenant?.billing_tier || '').toUpperCase();
        // 🛠️ MANUAL OVERRIDE for internal/test accounts that lack a Stripe
        // subscription. If an admin has manually set subscription_active=true
        // AND the billing_tier is a paid/admin tier, we treat them as active —
        // even when there is no `subscriptions` row at all (sub is null).
        // This is what unblocks your test tenants without forcing a live charge.
        const PAID_TIERS = new Set(['RETAIL', 'PRO', 'INSTITUTIONAL', 'ADMIN']);
        const manualOverride = tenant?.subscription_active === true && PAID_TIERS.has(tier);

        // Hard stop: explicit cancel / dunning states. Manual override can re-enable
        // these only if an admin ALSO flipped subscription_active back on after.
        if (sub?.status && ['canceled', 'unpaid', 'incomplete_expired'].includes(sub.status)) {
            if (manualOverride) {
                return { active: true, reason: `Manual override (${tier})`, status: 'manual_override' };
            }
        ...
        if (sub?.status === 'trialing' && sub?.trial_end && new Date(sub.trial_end).getTime() < now) {
            if (manualOverride) {
                return { active: true, reason: `Manual override (${tier}) past trial`, status: 'manual_override' };
            }
        ...
        if (sub?.status === 'past_due' && sub?.current_period_end && new Date(sub.current_period_end).getTime() < now) {
            if (manualOverride) {
                return { active: true, reason: `Manual override (${tier}) past due`, status: 'manual_override' };
            }
        ...
        // 🛠️ No subscription row at all but admin has marked the tenant active
        // with a paid/admin tier → manual override permits trading.
        if (!sub && manualOverride) {
            return { active: true, reason: `Manual override (${tier})`, status: 'manual_override' };
        }
...
export async function deactivateTenantStrategies(tenantId, reason = 'BILLING') {
    const supabase = getTenantClient();
    try {
        const updatePayload = { is_active: false, updated_at: new Date().toISOString() };
        if (reason === 'BILLING' || reason.startsWith('Subscription') || reason.startsWith('Free trial')) {
            updatePayload.billing_paused = true;
        }
