export async function coinglass_spot_cvd_divergence_v4(symbol, interval) {
    ...
    try {
        ...
        const resSpot = await fetch(`https://open-api-v4.coinglass.com/api/spot/taker-buy-sell-volume/history?symbol=${symbol}${intervalParam}&limit=168`, {
            headers: { 'accept': 'application/json', 'CG-API-KEY': process.env.COINGLASS_API_KEY }
        });
        const resFut = await fetch(`https://open-api-v4.coinglass.com/api/futures/taker-buy-sell-volume/history?symbol=${symbol}${intervalParam}&limit=168`, {
            headers: { 'accept': 'application/json', 'CG-API-KEY': process.env.COINGLASS_API_KEY }
        });
