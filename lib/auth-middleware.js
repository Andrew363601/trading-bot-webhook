...
export async function verifyTenantContext(req) {
    try {
        ...
        const { data: userLink, error } = await supabase
            .from('tenant_users')
            .select('tenant_id, role, tenants(billing_tier, subscription_active, updated_at)')
        ...
        // Check subscription status with 3-day grace period
        const isSubscriptionActive = userLink.tenants?.subscription_active;
        const lastUpdate = new Date(userLink.tenants?.updated_at || 0).getTime();
        const gracePeriodMs = 3 * 24 * 60 * 60 * 1000; // 3 days
        const isWithinGracePeriod = (Date.now() - lastUpdate) < gracePeriodMs;

        const role = userLink.role;

        // Allow ADMIN to bypass, or active subscribers, or those in grace period
        if (role !== 'ADMIN' && !isSubscriptionActive && !isWithinGracePeriod) {
