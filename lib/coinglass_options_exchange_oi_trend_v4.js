export async function coinglass_options_exchange_oi_trend_v4(symbol) {
    ...
    try {
        const resOpt = await fetch(`https://open-api-v3.coinglass.com/api/option/oi/history?symbol=${symbol}`, {
            headers: { 'accept': 'application/json', 'CG-API-KEY': process.env.COINGLASS_API_KEY }
        });
        const resFut = await fetch(`https://open-api-v3.coinglass.com/api/futures/open-interest/history?symbol=${symbol}`, {
            headers: { 'accept': 'application/json', 'CG-API-KEY': process.env.COINGLASS_API_KEY }
        });
