...
export async function recordUsage(tenantId, metric, quantity = 1) {
    try {
        ...
        if (stripe) {
            ...
            // If they don't have a sub ID, we fallback to the master company customer ID
            const subscriptionId = sub?.stripe_subscription_id || process.env.STRIPE_COMPANY_SUBSCRIPTION_ID;

            if (subscriptionId) {
                // Determine Stripe Meter ID based on metric
                const meterMap = {
                ...
                if (meterId) {
                    await stripe.billing.meterEvents.create({
                        ...
                        payload: {
                            ...
                            stripe_customer_id: sub?.stripe_customer_id || process.env.STRIPE_COMPANY_CUSTOMER_ID
                        }
                    });
                }
            }
