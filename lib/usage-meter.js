...
export async function recordUsage(tenantId, metric, quantity = 1) {
    try {
        console.log(`[USAGE_METER] Recording ${metric} for tenant ${tenantId} (qty: ${quantity})`);
        ...
        if (logError) {
            console.error(`[USAGE_METER] Supabase insert failed for ${metric}:`, logError.message);
            throw logError;
        }
        ...
        if (stripe) {
            ...
            console.log(`[USAGE_METER] Subscription lookup result:`, sub ? `customer=${sub.stripe_customer_id} status=${sub.status}` : 'no subscription found');
