// lib/wix-content.js
// Fetches landing page content from Wix CMS with hardcoded fallback.
// The fallback ensures the page is never blank even if Wix is offline.
...
export const FALLBACK_CONTENT = {
  hero: {
    ...
    trialText: "7-Day Free Trial. Deploy your first autonomous agent in minutes."
  },
  features: [
...
export async function fetchWixContent() {
  try {
    const res = await fetch('/api/wix-content');
    const data = await res.json();
    if (data.configured && data.content) {
      return data.content;
    }
    return FALLBACK_CONTENT;
  } catch (e) {
    console.warn('[WIX] Using fallback:', e.message);
    return FALLBACK_CONTENT;
  }
}
