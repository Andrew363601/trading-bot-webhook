...
export async function buildRadarChartUrl({ asset, candles, poc, upperNode, lowerNode, currentPrice, trapPrice, trapSide, tpPrice, slPrice, openTrade }) {
    try {
        ...
        // PREVIOUSLY this used `type: 'candlestick'` which requires the
        // chartjs-chart-financial plugin — NOT loaded by QuickChart by default.
        // That made every chart-create call fail silently (URL came back null)
        // and the user never saw the embedded image. Switching to a plain line
        // chart of CLOSE prices is 100% reliable on QuickChart and the value of
        // these alerts is the OVERLAYED levels (entry / TP / SL / trap / nodes),
        // not the candle wicks.
        const closes = recentCandles.map(c => c.close);
        const lineColor = (closes.length > 1 && closes[closes.length - 1] >= closes[0])
            ? 'rgba(16, 185, 129, 1)'  // emerald — uptrend
            : 'rgba(239, 68, 68, 1)';  // red — downtrend
        ...
        const chartConfig = {
            ...
            data: {
                labels: labels,
                datasets: [{
                    label: `${asset} (Last ${closes.length} Closes)`,
                    data: closes,
